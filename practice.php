...
<script src="plotly.min.js" type="text/javascript"></script><style></style>
<script src="glow.min.js" type="text/javascript"></script>
</head>
<body>
<div class="glowscript" id="glowscript">
<script type="text/javascript">

var sock = '57624' // a port number generated and inserted here by no_notebook.py

window.__context = { glowscript_container: $("#glowscript") }
window.Jupyter_VPython = '/' // prefix used by glow.min.js for textures
...
function fontloading() {
	...
	if (window.__font_sans === undefined) {
        var fsans = 'Roboto-Medium.ttf' // a sans serif font; no_notebook server prepends vpython_data/
	...
	if (window.__font_serif === undefined) {
        var fserif = 'NimbusRomNo9L-Med.otf' // a serif font; no_notebook server prepends vpython_data/
...
window.onload = function() {
	...
	ws.onopen = function() {
		isopen = true
		//ws.send('trigger') // ask server to send data
	}
	...
	ws.onmessage = function(e) {
		...
		var t1 = msclock()
		if (e.data != 'trigger') {
			var msg = decode(JSON.parse(e.data))
			handler(msg)
		}
		var t2 = msclock()
	...
	ws.onerror = function(e) {
		console.log('Error in glowcomm.html')
	}
}
...
function awaitfonts() {
    "use strict";
	if (window.__font_sans === undefined ||
			window.__font_serif === undefined || !isopen) setTimeout(awaitfonts, 0)
	else ws.send('trigger') // ask server to send data
}
awaitfonts()
...
function send() { // periodically send events and update_canvas to websocket and request object update
	...
	if (isopen) {
		...
		var msg
		if (events.length > 0) msg = JSON.stringify(events)
		else msg = 'trigger'
		ws.send(msg)
		events = []
	}
...
<div style="white-space: pre;"></div><div style="white-space: pre;"></div><div class="glowscript-canvas-wrapper ui-resizable" style="height: 400px; display: inline-block; position: relative; float: none;"><canvas width="640" height="400" style="width: 640px; height: 400px; position: absolute;"></canvas><canvas width="640" height="400" style="width: 640px; height: 400px; position: relative; background-color: transparent;"></canvas><div class="ui-resizable-handle ui-resizable-e" style="z-index: 90;"></div><div class="ui-resizable-handle ui-resizable-s" style="z-index: 90;"></div><div class="ui-resizable-handle ui-resizable-se ui-icon ui-icon-gripsmall-diagonal-se" style="z-index: 90;"></div></div><div style="white-space: pre;"></div></div>
</body></html>
